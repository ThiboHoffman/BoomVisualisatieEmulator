...
public class CreateTreeConeTree : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        StartCoroutine(VisitInPreOrder());
    }
    ...
    public void VisualizeChildren2D<T>(TreeNode<T> CurrentNode, List<TreeNode<T>> waiting) {
        ...
        foreach (TreeNode<T> node in CurrentNode.Children) 
        {
            ...
            var Xposition = Mathf.Cos(angle) * radius * (_depth - CalculateDepthOfNode(node));
            var Zposition = Mathf.Sin(angle) * radius * (_depth - CalculateDepthOfNode(node));
            var pos = CurrentNode.Node.Obj.transform.position + new Vector3(Xposition, -1F, Zposition);
